...
function ResidentHome({ user, onLogout }) {
  ...
  const [data, setData] = useState({ announcements: [], events: [], documents: [], reservations: [], messages: [], guests: [], household: [], poolCards: [], poolAgreements: [] })
  ...
  const emptyHouseholdMember = { firstName: '', lastName: '', email: '', phone: '' }
  ...
  async function replyToMessage(id, reply) {
    ...
    setNotice('')
    try {
      await api(`/api/portal/messages/${id}/replies`, { method: 'POST', body: JSON.stringify({ reply }) })
  ...
  async function requestHouseholdAccess(event) {
    event.preventDefault(); setError(''); setNotice('')
    try {
      await api('/api/portal/household', { method: 'POST', body: JSON.stringify(householdMember) })
  ...
  return <div className="portal-shell"><PortalHeader user={user} /><main className="resident-dashboard">
    ...
    {tab === 'pool' && <section className="pool-workspace"><header><div><p className="portal-kicker">Resident pool</p><h2>Pool rules and access</h2></div>{data.poolAgreements.length > 0 ? <span className="agreement-status">Agreement on file</span> : <span className="agreement-status agreement-missing">No agreement on file</span>}</header><PoolRules /><div className="resident-pool-records"><section><h3>Access cards for your property</h3>{data.poolCards.map((card) => <div className="data-row" key={card.id}><div><strong>{card.cardNumber}</strong><small>{card.assignedName || 'Property card'}{card.notes ? ` | ${card.notes}` : ''}</small></div><span className={`status status-${card.status}`}>{card.status}</span></div>)}{data.poolCards.length === 0 && <p className="empty-state">No pool access cards are recorded for this property.</p>}</section><section><h3>Agreements on file</h3>{data.poolAgreements.map((agreement) => <div className="data-row" key={agreement.id}><div><strong>{agreement.signedByName}</strong><small>Rules version {agreement.rulesVersion}</small></div><span>{dateTime(agreement.acknowledgedAt)}</span></div>)}{data.poolAgreements.length === 0 && <p className="empty-state">An owner agreement has not been recorded for this property.</p>}</section></div></section>}
    ...
    {tab === 'messages' && <div className="resident-message-layout"><section className="editor-panel"><h2>Message the HOA</h2><p className="form-context">Your account and property will be included automatically.</p><form onSubmit={sendMessage}><label>Topic<select value={message.category} onChange={(event) => setMessage({ ...message, category: event.target.value })}><option value="general">General question</option><option value="maintenance">Community maintenance</option><option value="architectural">Architectural review</option><option value="board">HOA board</option></select></label><label>Message<textarea required maxLength="5000" value={message.message} onChange={(event) => setMessage({ ...message, message: event.target.value })} /></label><button className="primary-button" disabled={sendingMessage}>{sendingMessage ? 'Sending...' : 'Send message'}</button></form></section><section className="resident-message-history"><h2>Your conversations</h2>{data.messages.map((item) => <ResidentMessageThread item={item} dateTime={dateTime} onReply={replyToMessage} onDelete={deleteMessage} key={item.id} />)}{data.messages.length === 0 && <p className="empty-state">You have not sent any messages.</p>}</section></div>}
    {tab === 'household' && <div className="household-layout"><section className="editor-panel"><h2>Request household access</h2><p className="form-context">Add another person who lives at your property. The HOA must approve their account before they can sign in.</p><form onSubmit={requestHouseholdAccess}><div className="field-row"><label>First name<input required maxLength="80" value={householdMember.firstName} onChange={(event) => setHouseholdMember({ ...householdMember, firstName: event.target.value })} /></label><label>Last name<input required maxLength="80" value={householdMember.lastName} onChange={(event) => setHouseholdMember({ ...householdMember, lastName: event.target.value })} /></label></div><label>Email address<input required type="email" maxLength="254" value={householdMember.email} onChange={(event) => setHouseholdMember({ ...householdMember, email: event.target.value })} /></label><label>Phone number <span>Optional</span><input type="tel" maxLength="30" value={householdMember.phone} onChange={(event) => setHouseholdMember({ ...householdMember, phone: event.target.value })} /></label><button className="primary-button">Request access</button></form></section><section className="household-history"><h2>Household accounts</h2>{data.household.map((item) => <article className="household-record" key={item.id}><div><strong>{item.firstName} {item.lastName}</strong><small>{item.email}{item.phone ? ` | ${item.phone}` : ''}</small></div><span className={`status status-${item.status}`}>{item.status}</span></article>)}{data.household.length === 0 && <p className="empty-state">No household-member accounts have been requested.</p>}</section></div>}
    {tab === 'settings' && <section className="settings-panel"><h2>Account details</h2><dl className="account-details"><div><dt>Property</dt><dd>{user.address}</dd></div><div><dt>Property relationship</dt><dd>{residentTypeLabel(user.residentType)}</dd></div></dl><h2>Email notifications</h2><p>Choose which optional community updates are emailed to you. Account security and reservation decisions are always sent.</p><form onSubmit={savePreferences}><label><input type="checkbox" checked={preferences.notifyAnnouncements} onChange={(event) => setPreferences({ ...preferences, notifyAnnouncements: event.target.checked })} /><span><strong>Announcements</strong><small>Board notices and community updates</small></span></label><label><input type="checkbox" checked={preferences.notifyEvents} onChange={(event) => setPreferences({ ...preferences, notifyEvents: event.target.checked })} /><span><strong>Events</strong><small>New meetings and community events</small></span></label><button className="primary-button">Save preferences</button></form></section>}
  </main></div>
}
...
function ResidentMessageThread({ item, dateTime, onReply, onDelete }) {
  ...
  return <article className="resident-message-record"><header><span>{item.category}</span><span className={`status status-${item.status}`}>{item.status}</span></header><div className="message-thread"><div className="thread-entry from-resident"><small>You</small><p>{item.message}</p><time>{dateTime(item.createdAt)}</time></div>{(item.replies || []).map((entry) => <div className={`thread-entry from-${entry.authorRole}`} key={entry.id}><small>{entry.authorRole === 'admin' ? 'HOA administrator' : 'You'}</small><p>{entry.body}</p><time>{dateTime(entry.createdAt)}</time></div>)}</div><form className="thread-reply-form" onSubmit={async (event) => { event.preventDefault(); setSending(true); if (await onReply(item.id, reply)) setReply(''); setSending(false) }}><label>Reply<textarea required maxLength="5000" value={reply} onChange={(event) => setReply(event.target.value)} /></label><div className="thread-actions"><button className="primary-button" disabled={sending || !reply.trim()}>{sending ? 'Sending...' : 'Send reply'}</button><button type="button" className="row-delete" onClick={() => onDelete(item.id)}>Delete conversation</button></div></form></article>
}
        ...
        <section className="auth-panel">
          ...
          {mode === 'login' ? (
            ...
            <form onSubmit={submitRegistration}>
              ...
              <label>Property relationship<select value={registration.residentType} onChange={(event) => setRegistration({ ...registration, residentType: event.target.value, poolRulesAcknowledged: event.target.value === 'owner' ? registration.poolRulesAcknowledged : false })}><option value="owner">Property owner</option><option value="tenant">Renter</option></select></label>
              {registration.residentType === 'owner' && <div className="registration-pool-agreement"><details><summary>Review the pool rules and access-card guidelines</summary><PoolRules compact /></details><label className="rules-check"><input required type="checkbox" checked={registration.poolRulesAcknowledged} onChange={(event) => setRegistration({ ...registration, poolRulesAcknowledged: event.target.checked })} /><span>{POOL_ACKNOWLEDGEMENT}</span></label></div>}
