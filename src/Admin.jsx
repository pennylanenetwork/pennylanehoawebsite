...
function AccountReview({ account, currentUser, properties, onStatus, onSave }) {
  ...
  return <article className="account-review"><header><div><strong>{account.firstName} {account.lastName}</strong><small>{account.email} | {account.address}</small></div><div><span className={`status status-${account.status}`}>{account.status}</span><span className="account-type">{residentTypeLabel(account.residentType)}</span></div></header>{editing ? <form onSubmit={async (event) => { event.preventDefault(); if (await onSave(account.id, form)) setEditing(false) }}><div className="field-row"><label>First name<input required value={form.firstName} onChange={(event) => update('firstName', event.target.value)} /></label><label>Last name<input required value={form.lastName} onChange={(event) => update('lastName', event.target.value)} /></label></div><div className="field-row"><label>Email<input required type="email" value={form.email} onChange={(event) => update('email', event.target.value)} /></label><label>Phone<input value={form.phone} onChange={(event) => update('phone', event.target.value)} /></label></div><label>Property<select value={form.propertyId} onChange={(event) => update('propertyId', Number(event.target.value))}>{properties.map((property) => <option value={property.id} key={property.id}>{property.address} ({property.status})</option>)}</select></label><div className="field-row"><label>Property relationship<select value={form.residentType} onChange={(event) => update('residentType', event.target.value)}><option value="owner">Property owner</option><option value="tenant">Renter</option><option value="household_member">Household member</option></select></label><label>Access role<select value={form.role} disabled={account.id === currentUser?.id} onChange={(event) => update('role', event.target.value)}><option value="resident">Resident</option><option value="admin">Administrator</option>{currentUser?.role === 'super_admin' && <option value="super_admin">Super administrator</option>}</select></label></div><div className="form-actions"><button className="primary-button">Save resident</button><button type="button" className="quiet-button" onClick={() => setEditing(false)}>Cancel</button></div></form> : <div className="account-review-actions"><button type="button" onClick={() => setEditing(true)}>Edit</button>{account.status !== 'active' && <button type="button" onClick={() => onStatus(account.id, 'active')}>Approve</button>}{account.status === 'active' && account.id !== currentUser?.id && <button type="button" onClick={() => onStatus(account.id, 'suspended')}>Suspend</button>}{account.status === 'pending' && <button type="button" className="row-delete" onClick={() => onStatus(account.id, 'rejected')}>Reject</button>}</div>}</article>
}
