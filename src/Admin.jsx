...
export default function Admin() {
  ...
  return <div className="portal-shell admin-shell">
    <header className="portal-header">
      ...
      <nav className="portal-view-nav" aria-label="Switch portal view"><a href="/portal">Resident portal</a><a className="active" href="/admin">Administration</a></nav>
    </header>
    <main className="admin-main">
      ...
      </form>} rows={data.events.map((item) => <DataRow key={item.id} title={item.title} detail={dateTime(item.startsAt)} meta={item.status} actions={<div className="row-actions">{item.eventType !== 'clubhouse' && <><button type="button" onClick={() => beginEdit('event', item)}>Edit</button><button type="button" onClick={() => remove('events', item.id, 'Cancel this calendar event?')}>Cancel</button></>}{user?.role === 'super_admin' && <button type="button" className="row-delete" onClick={() => remove('events', `${item.id}/permanent`, 'Permanently delete this event and any linked reservation history? This cannot be undone.')}>Delete permanently</button>}</div>} />)} />}
      ...
      {tab === 'reservations' && <div><ClubhouseControls settings={data.clubhouse} blackouts={data.blackouts} onSave={saveClubhouseSettings} onAddBlackout={addBlackout} onDeleteBlackout={(id) => remove('clubhouse/blackouts', id, 'Remove this clubhouse blackout period?')} /><div className="reservation-admin-list">{data.reservations.map((item) => <ReservationReview key={item.id} item={item} onDecide={decideReservation} onCancel={() => remove('reservations', item.id, 'Cancel this clubhouse reservation?')} onDelete={user?.role === 'super_admin' ? () => remove('reservations', `${item.id}/permanent`, 'Permanently delete this reservation and its calendar event? This cannot be undone.') : null} />)}{data.reservations.length === 0 && <p className="empty-state">No clubhouse reservation requests yet.</p>}</div></div>}
      {tab === 'messages' && <div className="message-admin-list">{data.messages.map((item) => <MessageReview key={item.id} item={item} onUpdate={updateMessage} onReply={replyToMessage} onDelete={user?.role === 'super_admin' ? () => remove('messages', item.id, 'Permanently delete this message conversation and its communication history?') : null} />)}{data.messages.length === 0 && <p className="empty-state">No contact messages yet.</p>}</div>}
    </main>
  </div>
}
...
function MessageReview({ item, onUpdate, onReply, onDelete }) {
  ...
  return <article className={`message-review status-border-${item.status}`}><header><div><strong>{item.name}</strong><small><a href={`mailto:${item.email}`}>{item.email}</a>{item.phone ? ` | ${item.phone}` : ''}{item.address ? ` | ${item.address}` : ''}</small></div><span className={`status status-${item.status}`}>{item.status}</span></header><p className="message-category">{item.source} | {item.category} | {dateTime(item.createdAt)}</p><div className="message-thread admin-message-thread"><div className="thread-entry from-resident"><small>{item.name}</small><p>{item.message}</p><time>{dateTime(item.createdAt)}</time></div>{(item.replies || []).map((entry) => <div className={`thread-entry from-${entry.authorRole}`} key={entry.id}><small>{entry.authorRole === 'admin' ? 'HOA administrator' : item.name}</small><p>{entry.body}</p><time>{dateTime(entry.createdAt)}</time></div>)}</div><label>Reply to resident<textarea maxLength="5000" value={reply} onChange={(event) => setReply(event.target.value)} /></label><button type="button" className="primary-button message-reply-button" disabled={sending || !reply.trim()} onClick={async () => { setSending(true); if (await onReply(item.id, reply)) setReply(''); setSending(false) }}>{sending ? 'Sending...' : item.source === 'Resident portal' ? 'Send portal and email reply' : 'Email reply'}</button><label>Internal notes<textarea maxLength="3000" value={notes} onChange={(event) => setNotes(event.target.value)} /></label><div className="row-actions"><button type="button" onClick={() => onUpdate(item.id, 'read', notes)}>Mark read</button><button type="button" onClick={() => onUpdate(item.id, 'closed', notes)}>Close</button>{item.status === 'closed' && <button type="button" onClick={() => onUpdate(item.id, 'read', notes)}>Reopen</button>}{onDelete && <button type="button" className="row-delete" onClick={onDelete}>Delete permanently</button>}</div></article>
}
...
function AccountReview({ account, currentUser, properties, onStatus, onSave }) {
  ...
  const [form, setForm] = useState({
    ...
    isBoardMember: Boolean(account.isBoardMember), isAccMember: Boolean(account.isAccMember),
  })
  const update = (field, value) => setForm({ ...form, [field]: value })
  return <article className="account-review"><header><div><strong>{account.firstName} {account.lastName}</strong><small>{account.email} | {account.address}</small></div><div><span className={`status status-${account.status}`}>{account.status}</span><span className="account-type">{residentTypeLabel(account.residentType)}</span>{account.role !== 'resident' && <span className="account-role">{account.role === 'super_admin' ? 'Super administrator' : 'Administrator'}</span>}{account.isBoardMember && <span className="committee-role">Board member</span>}{account.isAccMember && <span className="committee-role">ACC Committee</span>}</div></header>{editing ? <form onSubmit={async (event) => { event.preventDefault(); if (await onSave(account.id, form)) setEditing(false) }}><div className="field-row"><label>First name<input required value={form.firstName} onChange={(event) => update('firstName', event.target.value)} /></label><label>Last name<input required value={form.lastName} onChange={(event) => update('lastName', event.target.value)} /></label></div><div className="field-row"><label>Email<input required type="email" value={form.email} onChange={(event) => update('email', event.target.value)} /></label><label>Phone<input value={form.phone} onChange={(event) => update('phone', event.target.value)} /></label></div><label>Property<select value={form.propertyId} onChange={(event) => update('propertyId', Number(event.target.value))}>{properties.map((property) => <option value={property.id} key={property.id}>{property.address} ({property.status})</option>)}</select></label><div className="field-row"><label>Property relationship<select value={form.residentType} onChange={(event) => update('residentType', event.target.value)}><option value="owner">Property owner</option><option value="tenant">Renter</option><option value="household_member">Household member</option></select></label><label>Access role<select value={form.role} disabled={currentUser?.role !== 'super_admin' || account.id === currentUser?.id} onChange={(event) => update('role', event.target.value)}><option value="resident">Resident</option><option value="admin">Administrator</option>{currentUser?.role === 'super_admin' && <option value="super_admin">Super administrator</option>}</select></label></div>{currentUser?.role === 'super_admin' && <div className="committee-options"><label className="rules-check"><input type="checkbox" checked={form.isBoardMember} onChange={(event) => update('isBoardMember', event.target.checked)} /><span>Board member</span></label><label className="rules-check"><input type="checkbox" checked={form.isAccMember} onChange={(event) => update('isAccMember', event.target.checked)} /><span>ACC Committee member</span></label></div>}<div className="form-actions"><button className="primary-button">Save resident</button><button type="button" className="quiet-button" onClick={() => setEditing(false)}>Cancel</button></div></form> : <div className="account-review-actions"><button type="button" onClick={() => setEditing(true)}>Edit</button>{account.status !== 'active' && <button type="button" onClick={() => onStatus(account.id, 'active')}>Approve</button>}{account.status === 'active' && account.id !== currentUser?.id && <button type="button" onClick={() => onStatus(account.id, 'suspended')}>Suspend</button>}{account.status === 'pending' && <button type="button" className="row-delete" onClick={() => onStatus(account.id, 'rejected')}>Reject</button>}{currentUser?.role === 'super_admin' && account.id !== currentUser.id && account.status === 'active' && account.role === 'resident' && <button type="button" className="role-button" onClick={() => onSave(account.id, { ...form, role: 'admin' })}>Make administrator</button>}{currentUser?.role === 'super_admin' && account.role === 'admin' && <button type="button" className="row-delete" onClick={() => onSave(account.id, { ...form, role: 'resident' })}>Remove administrator</button>}{currentUser?.role === 'super_admin' && <button type="button" onClick={() => onSave(account.id, { ...form, isBoardMember: !account.isBoardMember })}>{account.isBoardMember ? 'Remove from board' : 'Add to board'}</button>}{currentUser?.role === 'super_admin' && <button type="button" onClick={() => onSave(account.id, { ...form, isAccMember: !account.isAccMember })}>{account.isAccMember ? 'Remove from ACC' : 'Add to ACC'}</button>}</div>}</article>
}
...
function PhotoManager({ photos, onUpload, onUpdate, onDelete }) {
  ...
  return <div className="photo-workspace"><section className="editor-panel"><h2>Add carousel photo</h2><p className="form-context">{photos.length} of 15 photos stored. Hidden photos count toward the limit. Delete an existing photo before uploading when the gallery is full.</p><form onSubmit={submit}><label>Photo<input required disabled={atLimit} type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setFile(event.target.files[0] || null)} /><span>Images are resized and optimized before upload.</span></label><label>Alternative text<input required disabled={atLimit} maxLength="300" value={form.altText} onChange={(event) => setForm({ ...form, altText: event.target.value })} /><span>Briefly describe what is visible for residents using screen readers.</span></label><label>Caption<textarea disabled={atLimit} maxLength="500" value={form.caption} onChange={(event) => setForm({ ...form, caption: event.target.value })} /></label><button className="primary-button" disabled={busy || atLimit}>{atLimit ? '15-photo limit reached' : busy ? 'Optimizing...' : 'Upload photo'}</button></form></section><section className="photo-list">{photos.map((photo, index) => <PhotoEditor key={photo.id} photo={photo} canMoveUp={index > 0} canMoveDown={index < photos.length - 1} onMoveUp={() => move(index, -1)} onMoveDown={() => move(index, 1)} onUpdate={onUpdate} onDelete={() => onDelete(photo.id)} />)}{photos.length === 0 && <p className="empty-state">No carousel photos have been uploaded.</p>}</section></div>
}
...
function PropertyDetail({ propertyId, onBack, isSuperAdmin }) {
  ...
  const [error, setError] = useState('')
  const [notice, setNotice] = useState('')
  const load = () => api(`/api/admin/properties/${propertyId}`).then(setRecord).catch((requestError) => setError(requestError.message))
  ...
  async function addPoolCard(values) {
    setError(''); setNotice('')
    try { await api(`/api/admin/properties/${propertyId}/pool-cards`, { method: 'POST', body: JSON.stringify(values) }); setNotice('Pool card added.'); await load(); return true }
    catch (requestError) { setNotice(requestError.message); return false }
  }
  async function updatePoolCard(id, values) {
    setError(''); setNotice('')
    try { await api(`/api/admin/pool-cards/${id}`, { method: 'PATCH', body: JSON.stringify(values) }); setNotice('Pool card updated.'); await load() }
  ...
  async function deleteHistory(kind, id) {
    ...
    setError(''); setNotice('')
    try { await api(`/api/admin/history/${kind}/${id}`, { method: 'DELETE' }); setNotice('History record deleted.'); await load() }
    catch (requestError) { setError(requestError.message) }
  }
  if (error && !record) return <div><button type="button" className="quiet-button" onClick={onBack}>&larr; Properties</button><p className="form-error">{error}</p></div>
  if (!record) return <p className="empty-state">Loading property record...</p>
  const { property, residents: storedResidents, reservations, contacts, communications, poolCards, guests, audit } = record
  const residents = storedResidents.map((resident) => ({ ...resident, residentType: residentTypeLabel(resident.residentType) }))
  return <section className="property-detail"><header><button type="button" className="quiet-button" onClick={onBack}>&larr; Properties</button><div><p className="portal-kicker">Property record</p><h2>{property.address}</h2><p>{property.phase} | {property.city}, {property.state} {property.postalCode} | {property.status}</p></div></header>{notice && <p className="form-notice" role="status">{notice}</p>}{error && <p className="form-error" role="alert">{error}</p>}<div className="property-summary"><span><strong>{residents.length}</strong> registered accounts</span><span><strong>{poolCards.filter((card) => card.status === 'active').length}</strong> active pool cards</span><span><strong>{guests.filter((guest) => guest.status === 'active').length}</strong> guest registrations</span><span><strong>{reservations.length}</strong> reservations</span></div><PoolCardManager cards={poolCards} residents={residents} onAdd={addPoolCard} onUpdate={updatePoolCard} /><PropertySection title="Registered guests" empty="No guests are registered to this property.">{guests.map((guest) => <article className="property-guest" key={guest.id}><div><strong>{guest.guestName}</strong><small>{guest.startsOn} through {guest.endsOn} | Registered by {guest.registeredByName}</small></div><div><span className={`status status-${guest.status}`}>{guest.status}</span><small>Registered {dateTime(guest.createdAt)}</small><strong>{guest.poolResponsibilityAcknowledged ? 'Resident accepted responsibility for unsupervised pool access' : 'No pool responsibility acknowledgement'}</strong></div></article>)}</PropertySection><PropertySection title="Registered residents" empty="No accounts are linked to this property.">{residents.map((resident) => <article className="property-resident" key={resident.id}><div><strong>{resident.firstName} {resident.lastName}</strong><small>{resident.email}{resident.phone ? ` | ${resident.phone}` : ''}</small></div><div><span>{resident.residentType.replace('_', ' ')}</span><span>{resident.role}</span><span>{resident.status}</span></div></article>)}</PropertySection><PropertySection title="Communications" empty="No communications are linked to this property.">{communications.map((item) => <article className="communication-record" key={item.id}><header><span>{item.direction} {item.channel}</span><time>{dateTime(item.createdAt)}</time></header><strong>{item.subject}</strong><small>{item.correspondent} | {item.deliveryStatus}</small>{item.summary && <p>{item.summary}</p>}{isSuperAdmin && <button type="button" className="row-delete history-delete" onClick={() => deleteHistory('communications', item.id)}>Delete record</button>}</article>)}</PropertySection><PropertySection title="Website messages" empty="No website messages are linked to this property.">{contacts.map((item) => <article className="communication-record" key={item.id}><header><span>{item.category}</span><time>{dateTime(item.createdAt)}</time></header><strong>{item.name} | {item.status}</strong><p>{item.message}</p></article>)}</PropertySection><PropertySection title="Clubhouse reservations" empty="No clubhouse reservations are linked to this property.">{reservations.map((item) => <article className="property-list-row" key={item.id}><div><strong>{item.eventName}</strong><small>{item.residentName} | {dateTime(item.startsAt)}</small></div><span className={`status status-${item.status}`}>{item.status}</span></article>)}</PropertySection><PropertySection title="Record activity" empty="No recorded changes for this property.">{audit.map((item) => <article className="property-list-row" key={item.id}><div><strong>{item.action.replaceAll('.', ' ')}</strong><small>{item.actorName || 'System'} | {dateTime(item.createdAt)}</small></div><div className="history-record-actions"><span>{item.targetType}</span>{isSuperAdmin && <button type="button" className="row-delete" onClick={() => deleteHistory('audit', item.id)}>Delete record</button>}</div></article>)}</PropertySection></section>
}

function PoolCardManager({ cards, residents, onAdd, onUpdate }) {
  const empty = { cardNumber: '', assignedUserId: '', notes: '' }
  const [form, setForm] = useState(empty)
  return <section className="property-section pool-card-section"><h3>Pool access cards</h3><form className="pool-card-form" onSubmit={async (event) => { event.preventDefault(); if (await onAdd(form)) setForm(empty) }}><label>Card ID<input required maxLength="100" value={form.cardNumber} onChange={(event) => setForm({ ...form, cardNumber: event.target.value })} /></label><label>Assigned resident<select value={form.assignedUserId} onChange={(event) => setForm({ ...form, assignedUserId: event.target.value })}><option value="">Property only</option>{residents.map((resident) => <option value={resident.id} key={resident.id}>{resident.firstName} {resident.lastName}</option>)}</select></label><label>Notes<input maxLength="1000" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></label><button className="primary-button">Issue card</button></form><div className="pool-card-list">{cards.map((card) => <PoolCardRow card={card} residents={residents} onUpdate={onUpdate} key={card.id} />)}{cards.length === 0 && <p className="empty-state">No pool cards are recorded for this property.</p>}</div></section>
}

function PoolCardRow({ card, residents, onUpdate }) {
  const [form, setForm] = useState({ assignedUserId: card.assignedUserId || '', status: card.status, notes: card.notes || '' })
  return <article className="pool-card-row"><div><strong>{card.cardNumber}</strong><small>Issued {dateTime(card.issuedAt)}{card.assignedName ? ` | ${card.assignedName}` : ' | Property card'}</small></div><select aria-label={`Assigned resident for card ${card.cardNumber}`} value={form.assignedUserId} onChange={(event) => setForm({ ...form, assignedUserId: event.target.value })}><option value="">Property only</option>{residents.map((resident) => <option value={resident.id} key={resident.id}>{resident.firstName} {resident.lastName}</option>)}</select><select aria-label={`Status for card ${card.cardNumber}`} value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value })}><option value="active">Active</option><option value="lost">Lost</option><option value="stolen">Stolen</option><option value="returned">Returned / removed</option><option value="deactivated">Deactivated</option></select><input aria-label={`Notes for card ${card.cardNumber}`} maxLength="1000" placeholder="Notes" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /><button type="button" className="quiet-button" onClick={() => onUpdate(card.id, form)}>Save</button></article>
}
